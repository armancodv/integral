...
public class Complex {
     ...
     * @throws CalculatorException
     */
    public static Complex pow(final Complex c, final int exp) throws CalculatorException {
        double x = 0.0, y = 0.0;
        int sign;
        for (int i = 0; i <= exp; i++) {
            sign = ((i % 2) == 0) ? 1 : -1;
            // real
            x += Combination.calc(exp, 2 * i) * Math.pow(c.r, exp - (2 * i)) * Math.pow(c.i, 2 * i) * sign;
            if (exp == (2 * i)) {
                break;
            }
            // imaginary
            y += Combination.calc(exp, (2 * i) + 1) * Math.pow(c.r, exp - ((2 * i) + 1)) * Math.pow(c.i, (2 * i) + 1)
                    * sign;
        }
    ...
    public Complex pow(final Complex z) {
        final Complex a = mul(z, this.log());
        return a.exp();
    }
     ...
     * @return
     */
    public Complex pow(final double d) {
        final Complex a = mul(d, this.log());
        return a.exp();
    }
